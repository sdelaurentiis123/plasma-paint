...
from plasma_painter.generation.contract_audit import make_prompt, validate_candidate
...
def main():
    ...
    control=validate_candidate(reference,clip['frames'],config,{'name':'control'})
    if not control['accepted']: raise RuntimeError('Working control failed; aborting generation')
    ...
    for artist in ('Vincent van Gogh','Georges Seurat'):
        ...
        initial=make_prompt(clip['frames'][0],reference,example=True).replace('Van Gogh',artist)
        initial+='\nThe example is only an API starting point. Do NOT copy its painting construction. Invent different paths, widths, pressure, grouping, texture or media in response to the reference images and field. A palette-only edit does not count.'
        ...
        for attempt in range(4):
            ...
            item={'artist':artist,'attempt':attempt,'seed':1701,'generation_seed':generation_seed,'raw_text':raw,'program_hash':digest,'program_path':str(out/(digest+'.js')),
                  'prompt':text,'messages':chat,'duplicate_previous':code==previous_code,'image_count':len(images),'reference_ids':[r['id'] for r in refs],'validation':validity,'origin':'frozen_vision_model_complete_interface_example'}
            if validity['accepted']:
                item['construction']=check_construction(code,reference,clip['frames'])
            ...
            if 'render' in item and item.get('construction',{}).get('different_construction'):
                print(f'{artist}: non-copy construction rendered; requires human/fidelity review',flush=True)
                break
            if attempt<3:
                if 'render' in item:
                    images=images[:len(refs)+1]
                    with Image.open(item['render']['still']) as image:
                        image=image.convert('RGB');image.thumbnail((560,560));images.append(image.copy())
                    feedback='The LAST image is your rendered draft. Your mark construction copied the supplied example (color-only changes also fail this check). Replace the mark-construction algorithm, not merely formatting or palette. Choose new finite paths, layering, pressure, widths or media informed by the reference images. Preserve the data and all API bounds. Return a complete exported program.'
